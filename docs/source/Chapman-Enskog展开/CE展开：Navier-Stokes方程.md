...
**<font color='red'>红色为相对于Navier-Stokes方程增加的内容</font>**

## LBM求解对流扩散方程模型介绍
> 演化方程：

```{math}
:label: CE-conv-ns-eq1
...
```{math}
:label: CE-conv-ns-eq2
f_{i}^{\mathrm{eq}}(\boldsymbol{x},t)=w_iT\left[ 1+\frac{\boldsymbol{e}_{\boldsymbol{i}}\cdot \boldsymbol{u}}{c_{s}^{2}} +\frac{\left( \boldsymbol{e}_{\boldsymbol{i}}\cdot \boldsymbol{u} \right) ^2}{2c_{s}^{4}}-\frac{\boldsymbol{u}\cdot \boldsymbol{u}}{2c_{s}^{2}} \right]
```

> 宏观量计算：

```{math}
:label: CE-conv-ns-eq3
T=\sum_i{f_i}
```
...
其中$\boldsymbol{D}_i=\left( \partial_t+\boldsymbol{e}_i \cdot \nabla \right)$，将式{eq}`CE-conv-ns-eq4`，{eq}`CE-conv-ns-eq5`和{eq}`CE-conv-ns-eq6`代入上式得到：
```{math}
:label: CE-conv-ns-eq8
...
```{math}
:label: CE-conv-ns-eq9
A\varepsilon +B\varepsilon ^2=C\varepsilon +D\varepsilon ^2
```

则其中暗含了：$A=C$，$B=D$。

基于此准则，我们整理了式{eq}`CE-conv-ns-eq8`中$\varepsilon$的各阶项：

> $\varepsilon^1$
```{math}
:label: CE-conv-ns-eq10
...
根据式{eq}`CE-conv-ns-eq10`可以将式{eq}`CE-conv-ns-eq11`化简：
```{math}
:label: CE-conv-ns-eq12
...
同时，结合式{eq}`CE-conv-ns-eq3`和{eq}`CE-conv-ns-eq4`，我们使用基本准则，可以得到：
```{math}
:label: CE-conv-ns-eq13
\sum_i{f_{i}^{\left( n \right)}}=0 \quad \forall n\geqslant 1
```

对式{eq}`CE-conv-ns-eq10`求0阶矩得到：
```{math}
:label: CE-conv-ns-eq14
\frac{\partial T}{\partial t_1}{\color[RGB]{240, 0, 0} +\nabla _1\left( \boldsymbol{u}T \right) }=0
```

对式{eq}`CE-conv-ns-eq10`求1阶矩得到：
```{math}
:label: CE-conv-ns-eq15
...
对式{eq}`CE-conv-ns-eq12`求0阶矩得到：
```{math}
:label: CE-conv-ns-eq16
...
使用式{eq}`CE-conv-ns-eq15`化简上式得到：
```{math}
:label: CE-conv-ns-eq17
...
考虑式{eq}`CE-conv-ns-eq14`和式{eq}`CE-conv-ns-eq17`得到：
```{math}
:label: CE-conv-ns-eq18
