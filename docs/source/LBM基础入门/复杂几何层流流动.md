...
> [LBM结合半反弹边界+作用力模型求解Poiseuille流动](https://www.bilibili.com/video/BV1JwqYBDEiG/?share_source=copy_web&vd_source=6cb794fb9d90103946fb79ba5f60451f)
...
后续我们会整理LBM里所有常见的作用力模型，这里先讲最常用的一种——Guo等的作用力格式[[Z. Guo, C. Zheng, B. Shi, Discrete lattice effects on the forcing term in the lattice Boltzmann method, Phys. Rev. E. 65(4) (2002) 046308.]](https://doi.org/10.1103/PhysRevE.65.046308)。这种格式的使用很简单，只需要在LBM的演化方程里加一个源项就行：
...
$$\rho \boldsymbol{u} = \sum_\alpha f_\alpha \boldsymbol{e_\alpha} {\color[RGB]{240, 0, 0} + \frac{\delta_t}{2} \boldsymbol{F}}$$
...
下面展示主要结果：
 ...
 ```{figure} fig/fig5/圆柱绕流.png
:width: 50%
:alt: logo
:align: center
圆柱绕流模拟
```
 ```{figure} fig/fig5/随机多孔介质.png
:width: 50%
:alt: logo
:align: center
随机多孔介质模拟
```
 ```{figure} fig/fig5/稀奇古怪1.png
:width: 50%
:alt: logo
:align: center
两个平板模拟
```
 ```{figure} fig/fig5/稀奇古怪2.png
:width: 50%
:alt: logo
:align: center
形状JR的J模拟
...
还可以模拟泊肃叶流动，模型同样具有二阶精度
 ```{figure} fig/fig5/多孔介质泊肃叶流动1.jpg
:width: 50%
:alt: logo
:align: center
作用力驱动+任意几何模拟泊肃叶流动
```
```{figure} fig/fig5/多孔介质泊肃叶流动2.png
:width: 50%
:alt: logo
:align: center
...
[5.3任意几何流动模拟.zip](https://raw.githubusercontent.com/JR-LBM-THU/JRLBM-tutorial/main/docs/source/LBM基础入门/files/5.33任意几何流动.zip)
