...
从上图中我们还可以知道，对于1.5节中的泊肃叶流动，我们在$x$方向上的格点数对于全反弹是$n+1$个，而对于半反弹则是$n$个。下面展示了一个使用半反弹边界条件模拟泊肃叶流动的结果，可以看到其与理论解吻合的同样非常好，且模型仍具有二阶精度。计算算例与1.5节中的全反弹算例完全相同，仅边界条件进行了更改，从全反弹改为半反弹。
